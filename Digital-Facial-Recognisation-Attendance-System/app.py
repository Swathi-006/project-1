...
def create_default_admin():
    ...
    c.execute('SELECT * FROM users WHERE username = ?', ('swathi',))
    if not c.fetchone():
        # Create default admin (username: admin, password: admin123)
        hashed_password = hashlib.sha256('vcet@123'.encode()).hexdigest()
        now = datetime.datetime.utcnow().isoformat()
        c.execute('INSERT INTO users (username, password, role, created_at) VALUES (?, ?, ?, ?)',
                  ('swathi', hashed_password, 'admin', now))
        conn.commit()
        print(" Default admin created - Username: swathi, Password: vcet@123")
